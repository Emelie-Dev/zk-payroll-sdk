...
function contractEventActor(event: TypedContractEvent): string {
  ...
  switch (event.type) {
    ...
    case "registry_updated":
    case "registry_deactivated":
      return (e.employer as string) ?? "";
    case "committed":
    case "salary_revealed":
      return (e.employer as string) ?? "";
    case "payment_executed":
      return (e.recipient as string) ?? "";
    case "payment_scheduled":
      return (e.recipient as string) ?? "";
    case "payment_cancelled":
      return "";
...
function contractEventContractId(event: TypedContractEvent): string | undefined {
  return (event as unknown as Record<string, unknown>).contractId as string | undefined;
}

/** Derive ledger from a typed contract event. */
function contractEventLedger(event: TypedContractEvent): number | undefined {
  return (event as unknown as Record<string, unknown>).ledger as number | undefined;
}
...
export function normalizeAuditEvent(
  ...
  const eventType = (eventOrPayload as Record<string, unknown>).type;
  ...
  const rawEvent = (eventOrPayload as Record<string, unknown>).event;
